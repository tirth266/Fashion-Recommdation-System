...
class SizeEstimator:
    ...
    def __init__(self, model_path=None, proto_path=None):
        ...
        self.model_path = model_path or os.path.join(
            base_dir, "models", "pose_iter_440000.caffemodel"
        )
        self.proto_path = proto_path or os.path.join(
            base_dir, "models", "pose_deploy_linevec.prototxt"
        )
        self.net = None
        self._load_model()
    ...
    def estimate(
        self, image_bytes, user_height_cm=170.0, user_weight_kg=None, user_gender="male"
    ):
        """
        ...
        weight_kg = (
            float(user_weight_kg) if user_weight_kg else 70.0
        )  # Default fallback
        ...
        raw_confidence = (
            raw_confidence * 0.5 + (detected_cnt / 18.0) * 0.5
        )  # Blend conf
        ...
        anthro_measurements = self._predict_from_anthropometry(
        ...
        if raw_confidence < CONF_LOW:
            ...
            final_measurements["notes"] = (
                "Low image confidence. Used statistical averages."
            )
            ...
            method_used = "hybrid"
            final_measurements["notes"] = (
                "Blended image analysis with statistical data."
            )
        else:
        ...
        annotated_b64 = self._annotate_image(
            img.copy(), points, final_measurements, metrics_px, size_data, method_used
        )
        ...
        return {
            ...
            "measurements": {
                k: round(v, 1)
                for k, v in final_measurements.items()
    ...
    def _predict_from_anthropometry(self, height_cm, weight_kg, gender):
        ...
        if gender == "female":
            ...
            # Waist: Use waist-to-height ratio (WhtR) for better accuracy
            # Typical WhtR for healthy adult: 0.4-0.5, adjust by BMI
            whtr = 0.42 + (bmi - 22) * 0.01
            m["waist_width_cm"] = height_cm * whtr / 2.54  # Convert to half-width in cm
            ...
            # Waist: Use waist-to-height ratio with BMI adjustment
            # For male: typical WhtR 0.45-0.55 depending on BMI
            whtr = 0.46 + (bmi - 22) * 0.015
            m["waist_width_cm"] = height_cm * whtr / 2.54  # half-width in cm
    ...
    def _map_size(self, m, gender):
        ...
        if gender == "female":
            ...
            if est_circ < 80:
                return {"size": "XS (0-2)", "details": "Extra Small"}
            if est_circ < 88:
                return {"size": "S (4-6)", "details": "Small"}
            if est_circ < 96:
                return {"size": "M (8-10)", "details": "Medium"}
            if est_circ < 104:
                return {"size": "L (12-14)", "details": "Large"}
            return {"size": "XL (16+)", "details": "Extra Large"}
        else:
            # Men's Sizing (Chest Circumference cm)
            if est_circ < 86:
                return {"size": "XS", "details": "Extra Small (34)"}
            if est_circ < 94:
                return {"size": "S", "details": "Small (36-38)"}
            if est_circ < 102:
                return {"size": "M", "details": "Medium (38-40)"}
            if est_circ < 112:
                return {"size": "L", "details": "Large (42-44)"}
            if est_circ < 122:
                return {"size": "XL", "details": "Extra Large (46)"}
    ...
    def _detect_keypoints(self, img):
        ...
        blob = cv2.dnn.blobFromImage(
            img, 1.0 / 255, (in_w, in_h), (0, 0, 0), swapRB=False, crop=False
        )
    ...
    def _calculate_pixel_metrics(self, points, mask, contour):
        ...
        widths = []
        if r_wri:
            widths.append(get_width(r_wri[1]))
        if l_wri:
            widths.append(get_width(l_wri[1]))
        if widths:
            wrist_px = sum(widths) / len(widths)
    ...
    def _annotate_image(self, img, points, measurements, metrics_px, size_data, method):
        ...
        cv2.putText(
            img,
            f"Height: {measurements['height_cm']}cm",
            (30, 40),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.7,
            COLOR_TEXT,
            ...
            for p1, p2 in skeleton:
                try:
                    if points[p1] and points[p2]:
                        cv2.line(
                            img, points[p1], points[p2], (200, 200, 200), 2
                        )  # Light gray skeleton
